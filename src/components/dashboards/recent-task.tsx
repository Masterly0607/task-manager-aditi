'use client';
import { useRecentTask } from "@/features/dashboards/hook";
...
const RecentTasks = () => {
    const { data: tasks, isLoading } = useRecentTask();
                    ...
                    >
                        <div className="flex justify-center items-center gap-4 w-full">
                            ...
                            <div className="flex items-center gap-4 ml-auto">
                                <Badge
                                    variant="outline"
                                    className={cn("px-3 py-1 capitalize font-medium", statusStyles[task.status])}
                                >
                                    {task.status.replace("-", " ")}
                                </Badge>
                                <div className="flex items-center gap-1.5 min-w-20 justify-end">
                                    <Flag className={cn("h-4 w-4 fill-current", priorityColors[task.priority || "low"])}/>
